...
class MediaBrowseService {
  ...
  async getFavoritesForPlayer(player) {
    ...
    try {
      ...
      console.error(`Maxi Media Player: error getting favorites for player ${player.id}: ${JSON.stringify(e2)}`);
      return [];
    }
  }
...
class EntitiesEditor extends BaseEditor {
  ...
  render() {
    ...
    return this.editGroup > -1 ? ke`<mxmp-predefined-group-editor
...
class ArtworkOverridesEditor extends BaseEditor {
  render() {
    ...
    return this.editItem > -1 ? ke`<mxmp-artwork-override-editor
          .index=${this.editItem}
          .config=${this.config}
          .hass=${this.hass}
          @closed=${() => this.editItem = -1}
        ></mxmp-artwork-override-editor>` : ke`
          <div>
            Artwork Overrides
            <ha-control-button-group>
...
class Card extends h {
  ...
  render() {
    ...
    return ke`
      ...
      [PLAYER, () => ke` <mxmp-player .store=${this.store}></mxmp-player>`],
      [
        GROUPS,
        () => ke` <mxmp-groups
                        .store=${this.store}
...
class Grouping extends h {
  ...
  groupingButton(icon, click) {
    return ke` <mxmp-grouping-button @click=${click} .icon=${icon}></mxmp-grouping-button> `;
  }
  getNotJoinedPlayers() {
...
class Groups extends h {
  render() {
    ...
    return ke`
      ...
      return ke` <mxmp-group .store=${this.store} .player=${group} .selected=${selected}></mxmp-group> `;
    })}
      </mwc-list>
    `;
  }
